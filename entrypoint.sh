...
elif [[ "$DISCOVERY_SERVICE" == "kubernetes" ]]; then
  if [[ -z "${PRIMARY_NODE}" ]]; then
    cluster_join="${K8S_SERVICE_NAME}"
    echo "Joining cluster $cluster_join"
  else
    echo "Creating cluster $cluster_join"
  fi
else
...
[[ "${cluster_join/.*/}" == "$hostname" || "$cluster_join" == "$ipaddr" ]] && cluster_join=""
